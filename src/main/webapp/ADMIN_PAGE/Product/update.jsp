<%@page import="Lib.ExceptionUtils"%> <%@page import="Lib.Converter"%> <%@page import="DAOs.ProductDAO"%> <%@page import="DAOs.BrandDAO"%>
<%@page import="Models.Product"%> <%@page contentType="text/html" pageEncoding="UTF-8"%> <%@ taglib uri="http://java.sun.com/jsp/jstl/core"
prefix="c" %> <%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn"%> <%! BrandDAO bDAO = new BrandDAO(); %> <%! ProductDAO
pDAO = new ProductDAO(); %> <%! Product pd;%> <% pd = (Product) request.getAttribute("ProductUpdate");%> <% String queryString =
request.getQueryString(); boolean isError = ExceptionUtils.isWebsiteError(queryString); String exceptionMessage =
    ...
    <body>
                ...
                <div class="row">
                    ...
                    <form action="/Admin/Product/Update" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="price">
                            ...
                            <input type="text" name="txtProductPrice" value="<%=
                            Converter.convertMoneyToInteger(String.valueOf(pd.getStock().getPrice())).replace(".", ",")%>">
                        </div>
                        <div class="gender">
                            <fieldset>
                                ...
                                <input type="radio" name="rdoGender" value="Unisex" id="unisex" <%= (pd.getGender().equals("Unisex")) ?
                                "checked" : ""%>>
        ...
        <script>
            ...
            $("input[type=file][name=fileProductImg]").on("change", function () {
                let fname = $("input[type=file][name=fileProductImg]")
                    .val()
