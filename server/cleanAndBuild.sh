...
cd ~/Project/ && echo "Went into the /Project" && git stash && echo "Stashed" && git fetch origin && echo "fetched" && git pull && echo "Pulled newest changes" && mvn package -DskipTests && sudo rm -rf /opt/tomcat/updated/webapps/ROOT && sudo rm -f /opt/tomcat/updated/webapps/ROOT.war && echo "Removed the old files" && sudo cp -rf ./target/SQLproject-1.war /opt/tomcat/updated/webapps/ROOT.war && echo "Done!"
